        ...
        <>
            <div className="space-y-3 flex items-start flex-col justify-between w-full">
                <Visibility data={data}
                            disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.visibility)}/>
                <Status data={data} disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.status)}/>
                <DatePicker data={data} disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.date)}/>
                ...
                <div className="flex flex-row items-center gap-x-3 text-sm font-bold">
                    <AllReminder data={allReminder}
                                 disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.reminder_participant)}/>
                </div>
                <div className="flex flex-row items-center gap-x-3 text-sm font-bold">
                    <PersonalReminder data={personalReminder} schedule={data}
                                      disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.reminder_personal)}/>
                </div>
            </div>
            ...
            <div className="flex flex-row items-center gap-x-3 text-sm font-bold text-gray-400">
                ...
                <div className="flex flex-row items-center gap-x-1">
                    <Assignee data={data} participant={scheduleParticipant}
                              disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.assign)}>
          ...
          >
            ...
            <div className="font-bold text-gray-400 space-y-2">
                ...
                <Document data={data} document={documents}
                          disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.document)}/>
            </div>
            <FormInvite data={data} disabled={!checkSchedulePermission(data.extra_data, ScheduleAction.invite)}>
